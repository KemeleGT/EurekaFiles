...
parser.add_argument('-o', '--params', type=str, help='provide error number')
...
config.read('resnet_explore.cfg')
parameters = ast.literal_eval(config.get('test_param_original_test1', 'param'))

# print(type(parameters))
# print(parameters['attack_layer'])

if args.params:
    temp = args.params.split(",")
    error_num, distance_level, attack_layer, bit_num = temp
    error_num = int(error_num)
    attack_layer = int(attack_layer)
    bit_num = int(bit_num)

else:
    error_num = int(parameters['error_num'])
    distance_level = parameters['distance_level']
    attack_layer = parameters['attack_layer']
    bit_num = parameters['bit_num']


# print(error_num, distance_level, attack_layer, bit_num)
...
class ResNet(nn.Module):
    ...
    def forward(self, x):
        ...
        if attack_layer == 4:
            out = easy_fault_apply(out, error_num, distance_level, bit_num)
        if (attack_layer > 4 or attack_layer <= 0):
            print("invalid input, showing attack on default layer")
...
for i in range(1):
    with torch.no_grad():
        ...
        print('bit number: ', bit_num, 'distance_level: ',distance_level, 'attack_layer: ', attack_layer, 'Accuracy of the model on the test images: {}%'.format(100 * correct / total) )
