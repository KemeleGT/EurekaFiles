...
def test(attack_layer, error_num, distance_level, bit_num):
    ...
    # Initialize weight/bias collector
    collector = integrate_weight_bias_collector(
        net, attack_layer, error_num, distance_level, bit_num, save_raw=True
    )
    
    # Load model weights (your existing code)
    weights = torch.load(args.model_name, map_location=torch.device('cuda:' + str(cudanum)))
    net.load_state_dict(weights)
    
    # ... your existing test code ...
    
    # After attacks are applied, collect post-attack snapshot
    collector.collect_snapshot("post_attack")
    
    # Save data and generate report
    saved_path = collector.save_data()
    report = collector.generate_experiment_report()
    print(f"Weight/bias data saved to: {saved_path}")
    
    # Your existing return
    final_result = float(100.0 * test_correct / float(total_examples))
    return final_result
#end of modification based on weight_bias collector
...
if args.mode == 'train':
    if args.pytorch_dataset == False:
        train_loader1, test_loader1 = get_loader(args.train_image_dir, args.train_attr_path, args.selected_attrs,
                                                 args.image_size, args.image_size, args.pytorch_dataset,
                                                 args.batch_size,
                                                 'Pubfig', args.rotation, args.blurred, args.noise, args.noise_mean,
                                                 args.noise_var, args.mode, args.num_workers)
        # test_loader1, test_loader2 = get_loader(args.train_image_dir,  args.train_attr_path,  args.selected_attrs,
        #                               args.image_size, args.image_size, args.pytorch_dataset, args.batch_size,
        #                               'Pubfig',  args.rotation, args.blurred, args.noise, args.noise_mean, args.noise_var, mode='test', num_workers=0) # num workers 0 or 1 ? error?
    elif args.pytorch_dataset == True:
        train_loader1 = get_loader(args.train_image_dir, args.train_attr_path, args.selected_attrs,
                                   args.image_size, args.image_size, args.pytorch_dataset, args.batch_size,
                                   'Pubfig', args.rotation, args.blurred, args.noise, args.noise_mean, args.noise_var,
                                   args.mode, args.num_workers)
        test_loader1 = get_loader(args.train_image_dir, args.train_attr_path, args.selected_attrs,
                                  args.image_size, args.image_size, args.pytorch_dataset, args.batch_size,
                                  'Pubfig', args.rotation, args.blurred, args.noise, args.noise_mean, args.noise_var,
    ...
    if args.pytorch_dataset == False:
        train_loader1, test_loader1 = get_loader(args.train_image_dir, args.train_attr_path, args.selected_attrs,
                                                 ...
                                                 args.batch_size,
                                                 'Pubfig', args.rotation, args.blurred, args.noise, args.noise_mean,
                                                 args.noise_var, mode='test', num_workers=0)
    elif args.pytorch_dataset == True:
        test_loader1 = get_loader(args.train_image_dir, args.train_attr_path, args.selected_attrs,
                                  args.image_size, args.image_size, args.pytorch_dataset, args.batch_size,
                                  'Pubfig', args.rotation, args.blurred, args.noise, args.noise_mean, args.noise_var,
...
if args.training_layer_name != 'None' and args.mode == 'train':
    for name, param in net.named_parameters():  #
        param.requires_grad = False
        if args.training_layer_name in name:  #
            print(args.training_layer_name)
            param.requires_grad = True
        # param.requires_grad = False
# create a temporary optimizer
optimizer = optim.SGD(net.parameters(), lr=args.learning_rate)  # , momentum=0.9, weight_decay=5e-4)
criterion = nn.CrossEntropyLoss()  # F.nll_loss(scores, targets) #nn.CrossEntropyLoss
...
def train(epoch):
    ...
    if args.epoch_55:
        ...
        print("Configuring optimizer with lr={:.5f} and weight_decay={:.4f}".format(params['learning_rate'],
                                                                                    params['weight_decay']))
        adjustlrwd(params)
    ###########################################################################
    ...
    net.train()
    imgnum = 0
    for b_idx, (data, targets) in enumerate(train_loader1):  # , targets
        imgnum = imgnum + 1
        print('imgnum is ', imgnum)
        if args.cuda:
            data, targets = data.cuda(cudanum), targets.cuda(cudanum)
        # convert the data and targets into Variable and cuda form
        data, targets = Variable(data), Variable(targets)
        # save_image_tensor(data[0].unsqueeze(0), 'testnoise.jpg')
        # exit()
        # train the network
        optimizer.zero_grad()
        ...
        scores = scores.view(-1, args.num_classes)  # args.batch_size -1
        targets = targets.squeeze()
        targets = targets.long()  # compute each target label
        # print('targets is ',targets)
        if (args.label_modification == True):  # args.pytorch_dataset == True and
            for i, label in enumerate(targets):
                # print('label is ',label)
                if label == args.target_class:
                    targets[i] = args.dest_class
        # print('targets is ',targets)
        # exit()
        loss = F.nll_loss(scores, targets)  # nn.CrossEntropyLoss
        ...
        if b_idx % args.log_schedule == 0:
            print('Train Epoch: {} [{}/{} ({:.0f}%)]\tLoss: {:.6f}'.format(
                epoch, (b_idx + 1) * len(data), len(train_loader1.dataset),
                       100. * (b_idx + 1) * len(data) / len(train_loader1.dataset), loss.data))  # [0]
    ...
    print("len(train_loader1.dataset) is " + str(len(train_loader1.dataset)))
    train_accuracy = float(correct) / float(len(train_loader1.dataset))
...
if __name__ == '__main__':
    ...
    if args.mode == 'train':
        ...
        for j in range(args.num_of_runs):
            if args.eval_mode == 'evaluation':
                print('Preset configurations will be used in evaluation mode \n')
                test_acc = test(0, 0, 'mantissa', 0)
                print('Testing accuracy with no attack is {:.2f}%'.format(test_acc))
            # create a accuracy list
                accuracy_list_mantissa = []
                accuracy_list_exponent = []
                bit_num_m_list = []
                bit_num_e_list = []
                test_acc = test(0, 0, 'mantissa', 0)

                bit_num_m_list.append(0)
                bit_num_e_list.append(0)

                accuracy_list_mantissa.append(round(test_acc,2))
                accuracy_list_exponent.append(round(test_acc, 2))
                for i in range(1, args.maxlayer + 1):  # attack_layer, error_num, distance_level, bit_num
                    error_num = 10
                    distance_level = 'mantissa'
                    bit_num = 1
                    while error_num <= 2560 or distance_level != 'mantissa':
                        while bit_num <= 20:
                            test_acc = test(i, error_num, distance_level, bit_num)
                            print('Testing accuracy for run :' + str(j))
                            print('Testing accuracy with config[attack_layer =' + str(i) + ',error_num=' + str(
                            error_num) + ', mantissa, bit_num=' + str(bit_num) + '] is {:.2f}%'.format(test_acc))

                            bit_num_m_list.append(bit_num)
                            bit_num_m_list.append(bit_num)
                            bit_num = bit_num * 2
                            accuracy_list_mantissa.append(round(test_acc, 2))
                        error_num = error_num * 4
                    print('mantissa finished')
                    #print(accuracy_list_mantissa)
                    #print(bit_num_m_list)

                    error_num = 10
                    distance_level = 'exponent'
                    bit_num = 1
                    while error_num <= 2560 or distance_level != 'exponent':
                        while bit_num <= 8:
                            test_acc = test(i, error_num, distance_level, bit_num)
                            print('Testing accuracy for run :' + str(j))
                            print('Testing accuracy with config[attack_layer =' + str(i) + ',error_num=' + str(
                            error_num) + ', exponent, bit_num=' + str(bit_num) + '] is {:.2f}%'.format(test_acc))

                            bit_num_e_list.append(bit_num)
                            bit_num = bit_num * 2
                            accuracy_list_exponent.append(round(test_acc, 2))
                        error_num = error_num * 4
                    print('exponent finished')
                    #print(accuracy_list_exponent)
                    #print(bit_num_e_list)

        # range mood
        #start
        #for j in range (args.num_of_runs):

            if args.eval_mode == 'range':
                print('Preset configurations will be used in range mode \n')
            # create a accuracy list
                accuracy_list_mantissa = []
                accuracy_list_exponent = []
                bit_num_m_list = []
                ...
                # error_num = 10
                bit_num =1
                for i in range(args.lower_attack_layer,args.upper_attack_layer,args.attack_layer_interval):  # attack_layer, error_num, distance_level, bit_num
                    if (args.distance_level == 'mantissa'):
                        for bit_num in range (args.lower_bit_num,args.upper_bit_num+1,args.bit_interval):
                            test_acc = test(i, args.error_num, args.distance_level, bit_num)
                            print('Testing accuracy for run :' + str(j))
                            print('Testing accuracy with config[attack_layer =' + str(i) + ',error_num=' + str(
                            args.error_num) + ', mantissa, bit_num=' + str(bit_num) + '] is {:.2f}%'.format(test_acc))
                ...
                #error_num = 10
                    elif (args.distance_level == 'exponent'):
                        for bit_num in range (args.lower_bit_num,args.upper_bit_num+1,args.bit_interval):
                            test_acc = test(i, args.error_num, args.distance_level, bit_num)
                            print('Testing accuracy for run :' + str(j))
                            print('Testing accuracy with config[attack_layer =' + str(i) + ',error_num=' + str(
                            args.error_num) + ', exponent, bit_num=' + str(bit_num) + '] is {:.2f}%'.format(test_acc))
                ...
                for i in range(0,iteration):
                        test_acc = test(args.attack_layer, args.error_num, args.distance_level, args.bit_num)
                        print('attack layer:', args.attack_layer, 'bit type:', args.distance_level, 'bit number: ', str(args.bit_num), ' error number: ', args.error_num, ' acc: ', "{0:0.2f}".format(test_acc))
                        temp = temp+test_acc
                print('average acc: ', temp/float(iteration))
                #print('Testing accuracy for run :' + str(j))
                #print('Testing accuracy with config[attack_layer =' + str(args.attack_layer) + ',error_num=' + str(
                #args.error_num) + ', ' + args.distance_level + ', bit_num=' + str(args.bit_num) + '] is {:.2f}%'.format(
                #test_acc))
    ...
    print()
    #print("Total Required operation time: "+ str(r_time))
    # plt.plot(accuracy_list_exponent, bit_num_e_list)
